...
export function RegisterPage({ dict, lang }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await registerUser({ email, password, firstName, lastName, phone: phone ? `+91${phone}` : undefined });
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input type="text" placeholder="Priya" value={firstName} onChange={(e) => setFirstName(e.target.value)} required data-testid="register-firstName" />
            </div>
            <div>
              <label className="block text-sm font-medium text-ink-700 mb-1.5">{dict.auth.lastName}</label>
              <Input type="text" placeholder="Sharma" value={lastName} onChange={(e) => setLastName(e.target.value)} required data-testid="register-lastName" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex">
              <span className="inline-flex items-center px-3 rounded-l-lg border border-r-0 border-ink-200 bg-ink-50 text-sm text-ink-500">+91</span>
              <Input
                type="tel"
