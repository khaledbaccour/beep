...
export function OverviewTab({ d, lang }: TabProps) {
  ...
  const totalEarnings = completed.reduce((sum, b) => sum + b.amountPaise, 0);

  const stats = [
    { label: d.upcomingSessions, value: loading ? '—' : String(upcoming.length), icon: CalendarDays },
    { label: d.totalEarnings, value: loading ? '—' : `₹${paiseToINR(totalEarnings)} INR`, icon: DollarSign },
