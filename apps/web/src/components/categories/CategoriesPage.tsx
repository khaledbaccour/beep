...
interface CategoryData {
  ...
  descKey: 'fitnessDesc' | 'educationDesc' | 'designDesc' | 'lawDesc' | 'businessDesc' | 'technologyDesc' | 'coachingDesc' | 'languagesDesc';
...
export function CategoriesPage({ dict, lang }: Props) {
  ...
  return (
    <div className="bg-cream-300 min-h-screen">
      {/* Hero Header — tight, editorial */}
      <section className="pt-28 pb-6 sm:pt-32 sm:pb-8 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-display font-semibold text-ink-950 leading-[1.05] tracking-tight">
      ...
      {/* Categories — dense masonry-like grid, no gaps */}
              ...
              return (
                <a
                  ...
                  className={`group relative overflow-hidden transition-all duration-500 ease-out ${
                    // Borders: right border for left column items, bottom border for all except last row
                    isEven && !isLast ? 'md:border-r-2 md:border-ink-900' : ''
                  } ${
                    !isLast && !isSecondLast ? 'border-b-2 border-ink-900' : ''
                  } ${
                    // On mobile, every item except last has bottom border
                    isLast ? '' : 'max-md:border-b-2 max-md:border-ink-900'
                  }`}
                  style={{
