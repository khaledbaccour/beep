...
export function SupportPageContent({ dict, lang }: Props) {
  const d = dict.supportPage;

  const categories = [
    { title: d.cat1Title, desc: d.cat1Desc, icon: Rocket, bg: 'bg-brand-50', iconBg: 'bg-brand-100', iconColor: 'text-brand-600' },
    { title: d.cat2Title, desc: d.cat2Desc, icon: CalendarDays, bg: 'bg-[#FFF5F2]', iconBg: 'bg-[#FDE8E4]', iconColor: 'text-[#E87C6A]' },
    { title: d.cat3Title, desc: d.cat3Desc, icon: Wallet, bg: 'bg-emerald-50', iconBg: 'bg-emerald-100', iconColor: 'text-emerald-600' },
    { title: d.cat4Title, desc: d.cat4Desc, icon: Briefcase, bg: 'bg-amber-50', iconBg: 'bg-amber-100', iconColor: 'text-amber-600' },
    { title: d.cat5Title, desc: d.cat5Desc, icon: Settings, bg: 'bg-violet-50', iconBg: 'bg-violet-100', iconColor: 'text-violet-600' },
    { title: d.cat6Title, desc: d.cat6Desc, icon: Shield, bg: 'bg-cream-50', iconBg: 'bg-cream-100', iconColor: 'text-cream-700' },
  ];
    ...
    <>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((cat) => {
              ...
              return (
                <div
                  key={cat.title}
                  className={`${cat.bg} border-2 border-ink-900 rounded-2xl p-7 shadow-retro hover:shadow-retro-md hover:-translate-y-1 active:shadow-retro-sm active:translate-y-0 transition-all duration-200`}
                >
                  <div className={`w-12 h-12 rounded-full ${cat.iconBg} flex items-center justify-center border-2 border-ink-900 shadow-retro-sm mb-5`}>
                    <Icon size={22} className={cat.iconColor} strokeWidth={2} />
                  </div>
                  <h3 className="text-lg font-bold text-ink-900 mb-2">{cat.title}</h3>
                  <p className="text-sm text-ink-500 leading-relaxed">{cat.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="py-16 sm:py-24 bg-white">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 text-center">
          <div className="border-2 border-ink-900 rounded-2xl p-10 shadow-retro-md bg-gradient-to-br from-brand-50 to-white">
            <div className="w-16 h-16 rounded-full bg-brand-100 flex items-center justify-center border-2 border-ink-900 shadow-retro-sm mx-auto mb-6">
            ...
            <a
              href="mailto:contact@kliik.click"
            ...
            <p className="text-xs text-ink-400 mt-4">{d.responseTime}</p>
          </div>
        </div>
      </section>
